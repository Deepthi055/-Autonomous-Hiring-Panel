...
class ConsensusAgent {
  ...
  constructor(resume, transcript, jobDescription) {
    if (!jobDescription || typeof jobDescription !== "string") {
      console.warn("ConsensusAgent: jobDescription missing or invalid. Using default.");
  ...
  /**
   * Alias for computeWeightedScore to maintain compatibility with older orchestrator calls.
   * @deprecated Use computeWeightedScore instead.
   * @param {Array<Object>} agentResults - Individual agent evaluation results
   * @returns {number} Weighted score (0–10)
   */
  calculate(agentResults) {
    console.warn("[ConsensusAgent] The 'calculate' method is deprecated. Using 'computeLocalConsensus' fallback.");
    const result = this.computeLocalConsensus(agentResults);
    // Return object shape compatible with legacy orchestrator expectations if needed
    return {
